 ...
 * text-foreground / border-border) so the wheel sheet always matches the
 * surrounding dialog in BOTH light and dark mode — it must never render as a
 * flat light sheet over a dark dialog.
 *
...
export default function WheelSheet({ open, onOpenChange, trigger, label, children }) {
  ...
  if (isMobile) {
    return (
      <Drawer open={open} onOpenChange={onOpenChange}>
        ...
        <DrawerContent className="bg-background text-foreground border-border pb-safe">
          {label && (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="bg-background text-foreground border-border sm:max-w-sm">
        {label && (
          <DialogHeader>
