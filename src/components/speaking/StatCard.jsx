  ...
  return (
    <div className="rounded-xl border border-[#D6DAE3] bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium uppercase tracking-wide text-[#5A6781]">{label}</span>
        <span className={`flex h-8 w-8 items-center justify-center rounded-lg ${TONES[tone]}`}>
          {Icon && <Icon className="h-4 w-4" />}
        </span>
      </div>
      <p className="mt-2 font-display text-3xl font-semibold text-[#1B2A4B]">{value}</p>
    </div>
  );
}