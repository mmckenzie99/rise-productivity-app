    ...
    <Dialog open={!!trip} onOpenChange={(v) => !v && requestClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-2">
          ...
          {/* Travel Schedule */}
          <div className="rounded-lg border border-border bg-card p-4">
            <h3 className="font-display text-sm font-semibold text-foreground mb-2">Travel Schedule</h3>
          ...
          {/* Travel Details */}
          <div className="rounded-lg border border-border bg-card p-4">
            <h3 className="font-display text-sm font-semibold text-foreground mb-2">Travel Details</h3>
            {trip.travel_entries?.length ? (
                ...
                <div key={i} className="space-y-1 border-b border-[#E8EAF0] pb-2 mb-2 last:border-0 last:mb-0 last:pb-0">
                  ...
                  {entry.receipt?.url && (
                    <div className="flex items-center gap-2 py-1.5">
                      ...
                      <span className="text-sm text-muted-foreground min-w-[120px]">Receipt</span>
              ...
              <p className="text-sm text-muted-foreground">No travel details.</p>
            )}
          </div>

          {/* Lodging Details */}
          <div className="rounded-lg border border-border bg-card p-4">
            <h3 className="font-display text-sm font-semibold text-foreground mb-2">Lodging Details</h3>
            {trip.lodging_entries?.length ? (
              ...
              <p className="text-sm text-muted-foreground">No lodging details.</p>
            )}
          </div>

          {/* Per Diem */}
          <div className="rounded-lg border border-border bg-card p-4">
            <h3 className="font-display text-sm font-semibold text-foreground mb-2">Per Diem</h3>
              ...
              <div className="space-y-1">
                {trip.per_diem_days.map((d, i) => (
                  <div key={i} className="flex justify-between border-b border-[#E8EAF0] py-1 text-sm last:border-0">
                    <span className="text-foreground">{d.date} · {d.type}</span>
                    <span className="font-medium text-foreground">{formatCurrency(d.amount)}</span>
                  </div>
                ))}
              </div>
            ) : <p className="text-sm text-muted-foreground">No per diem days.</p>}
            ...
            <div className="flex justify-between pt-2">
              ...
              <span className="text-sm font-semibold text-foreground">{formatCurrency(trip.total_per_diem)}</span>
            </div>
          </div>

          {/* Cost Summary */}
          <div className="rounded-lg border border-border bg-card p-4">
            <h3 className="font-display text-sm font-semibold text-foreground mb-2">Cost Summary</h3>
            <div className="space-y-1.5 text-sm">
              {travelByType.Flight > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Airfare</span>
                  <span className="font-medium text-foreground">{formatCurrency(travelByType.Flight)}</span>
                </div>
              )}
              {travelByType.Rental > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Rental</span>
                  <span className="font-medium text-foreground">{formatCurrency(travelByType.Rental)}</span>
                </div>
              )}
              {travelByType['Personal Auto'] > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Personal Auto</span>
                  <span className="font-medium text-foreground">{formatCurrency(travelByType['Personal Auto'])}</span>
                </div>
              )}
              {lodgingTotal > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Lodging</span>
                  <span className="font-medium text-foreground">{formatCurrency(lodgingTotal)}</span>
                </div>
              )}
              <div className="flex justify-between border-t border-[#E8EAF0] pt-1.5">
                <span className="text-muted-foreground">Per Diem</span>
                <span className="font-medium text-foreground">{formatCurrency(trip.total_per_diem)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Button variant="outline" onClick={openChat} className="border-[#1B2A4B] bg-[#1B2A4B] text-white hover:bg-[#2A3D6B]"><MessageCircle className="mr-1.5 h-4 w-4" />Chat</Button>
