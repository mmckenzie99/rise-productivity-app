...
export default function PlanCommentsSection({ planId, planTitle, planDate, admins, currentUserId }) {
  const { user } = useAuth();
  const { items, loading, add, remove } = usePlanComments(planId);
      ...
      if (notifyAdmin?.email) {
        try {
          await base44.entities.Notification.create({
            engagement_id: planId,
            engagement_title: planTitle || 'Plan',
            speaking_date: planDate || undefined,
        ...
        try {
          await base44.integrations.Core.SendEmail({
            ...
            subject: `New comment on plan: ${planTitle || 'Plan'}`,
            body: `<div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:0 auto;color:#1B2A4B"><h2 style="font-family:Fraunces,Georgia,serif;color:#1B2A4B">${esc(author)} commented on a plan</h2><p style="font-size:16px"><strong>${esc(planTitle || 'Plan')}</strong></p><div style="border:1px solid #E3E6EC;border-radius:8px;padding:12px;background:#F7F8FA;font-size:14px;white-space:pre-wrap">${esc(draft)}</div><p style="font-size:13px;color:#5A6781;margin-top:12px">Open the Engagement Log calendar to view and reply.</p></div>`,
          });
        } catch (e) {
