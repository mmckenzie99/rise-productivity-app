...
export default function NewChatDialog({ open, onClose, onCreated, currentUser, existingRooms, initialLinkType, initialLinkedId }) {
  ...
  // Roster comes from the listParticipants backend function (service role),
  // which returns every user's id/name/role — fixing the empty-picker bug where
  // non-owners (and even admins) got nothing from User.list().
  useEffect(() => {
    if (!open) return;
    ...
    base44.functions
      ...
      .then((res) => setUsers((res?.data?.users || []).filter((x) => x.id !== currentUser.id)))
      .catch(() => setUsers([]))
  ...
  const create = async () => {
    ...
    // Instant open for an existing direct 2-person room (skips a round-trip).
    ...
    setError('');
    try {
      const res = await base44.functions.invoke('createChatRoom', {
        participantIds: partIds,
        linkType,
        linkedId: linkType !== 'none' ? linkedId : undefined,
        title,
        topic,
      });
      const data = res?.data || {};
      ...
      if (data.room) onCreated(data.room);
    } catch (e) {
      setError(e?.response?.data?.error || e?.message || 'Failed to create conversation');
    } finally {
      setCreating(false);
    }
  };

  const canCreate = canStart && selectedUsers.size > 0 && (linkType === 'none' || !!linkedId);
        ...
        {!canStart ? (
          ...
          <div className="space-y-4">
            <div>
              ...
              {usersLoading ? (
                ...
                <div className="max-h-40 space-y-1 overflow-y-auto rounded-md border border-border p-2">
                  {users.map((u) => (
                    <label key={u.id} className="flex items-center gap-2.5 rounded px-1 py-1 hover:bg-accent">
            ...
            <div>
              ...
              <ResponsiveSelect
                value={linkType}
                onValueChange={(v) => { setLinkType(v); setLinkedId(''); }}
                options={[{ value: 'none', label: 'No link' }, { value: 'engagement', label: 'Engagement' }, { value: 'trip', label: 'Trip' }, { value: 'plan', label: 'Plan' }]}
                placeholder="No link"
            ...
            {linkType === 'engagement' && (
              <ResponsiveSelect value={linkedId} onValueChange={setLinkedId} options={engagements.map((e) => ({ value: e.id, label: e.title || e.place || 'Untitled' }))} placeholder="Select engagement" label="Select engagement" />
            )}
            {linkType === 'trip' && (
              <ResponsiveSelect value={linkedId} onValueChange={setLinkedId} options={trips.map((t) => ({ value: t.id, label: formatPlaces(t) }))} placeholder="Select trip" label="Select trip" />
            )}
            {linkType === 'plan' && (
              <ResponsiveSelect value={linkedId} onValueChange={setLinkedId} options={plans.map((p) => ({ value: p.id, label: `${p.title}${p.date ? ` · ${p.date}` : ''}` }))} placeholder="Select plan" label="Select plan" />
            )}

            {linkType === 'none' && selectedUsers.size > 1 && (
              <div>
                <Label className="mb-1.5 block text-sm">Group name (optional)</Label>
                <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Group chat" />
              </div>
            )}

            <div>
              <Label className="mb-1.5 block text-sm">Topic (optional)</Label>
              <Input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="e.g. Q3 travel planning" />
            </div>
